# JAMF Pro Deployment Guide for unwarp Emergency Tool

## Overview

This guide provides step-by-step instructions for deploying the unwarp emergency tool through JAMF Pro. The unwarp tool is an emergency utility that allows authorized IT staff to disconnect Cloudflare WARP when normal administrative access is unavailable.

**Important**: This tool should only be deployed to designated incident response personnel, not to all users.

## Getting the Package

Download pre-built packages from the [unwarp releases page](https://github.com/aberoham/unwarp/releases/latest). Choose the appropriate package for your deployment:
...
- A unwarp installer package downloaded from [GitHub Releases](https://github.com/aberoham/unwarp/releases/latest)
...
4. Fill in the package information:
   - **Display Name**: unwarp Emergency Tool
   - **Filename**: Use the package filename from GitHub (e.g., unwarp-2025.6.1.0-universal.pkg)
   ...
   - **Package ID**: com.github.aberoham.unwarp (or your organization's identifier)
5. Click **Save**
...
### General Tab
- **Display Name**: Deploy unwarp Emergency Tool
- **Enabled**: Yes
- **Category**: Utilities
...
2. Set **Display Name**: Install unwarp Emergency Tool
3. Add **Description**: 
   ```
   Installs the unwarp emergency tool for disconnecting Cloudflare WARP 
...
1. Under **Files and Processes**, you can add:
   - **Execute Command**: 
     ```bash
     /usr/bin/logger -t unwarp "Emergency tool deployed via JAMF to $(whoami)"
     ```
...
3. Verify installation:
   ```bash
   ls -la /usr/local/libexec/.unwarp
   ```
   Should show: `-rwsr-xr-x  1 root  wheel  [size] [date] .unwarp`

## Step 5: Monitoring and Compliance

### Creating an Extension Attribute
To track which computers have unwarp installed:

1. Navigate to **Computer Management** → **Extension Attributes**
2. Click **New**
3. Configure:
   - **Display Name**: unwarp Installed
   - **Data Type**: String
   - **Input Type**: Script
   - **Script**:
     ```bash
     #!/bin/bash
     if [ -f "/usr/local/libexec/.unwarp" ]; then
...
2. Create a search for:
   - Criteria: "unwarp Installed" is "Installed"
...
- **Hidden Location**: Tool is installed in a hidden location (note the dot in `.unwarp`)
...
- If needed, create a policy to run:
  ```bash
  chown root:wheel /usr/local/libexec/.unwarp
  chmod 4755 /usr/local/libexec/.unwarp
  ```

### Tool Not Working
- Verify Touch ID is enabled on the device
- Check system logs: `log show --predicate 'subsystem == "unwarp"' --last 1h`
...
- **unwarp tool itself**: Review logs and contact your security team
